...
log2file()
{
  ...
  echo $msg >> $wittypi_home/Sys2RTC.log
}
...
log '  Writing system time to RTC...'
sys_ts=$(calc $(get_sys_timestamp)+1)
sec=$(date -u -d @$sys_ts +%S)
min=$(date -u -d @$sys_ts +%M)
hour=$(date -u -d @$sys_ts +%H)
day=$(date -u -d @$sys_ts +%u)
date=$(date -u -d @$sys_ts +%d)
month=$(date -u -d @$sys_ts +%m)
year=$(date -u -d @$sys_ts +%y)
i2c_write 0x01 $I2C_RTC_ADDRESS 0x00 $(dec2bcd $sec)
i2c_write 0x01 $I2C_RTC_ADDRESS 0x01 $(dec2bcd $min)
i2c_write 0x01 $I2C_RTC_ADDRESS 0x02 $(dec2bcd $hour)
i2c_write 0x01 $I2C_RTC_ADDRESS 0x03 $(dec2bcd $day)
